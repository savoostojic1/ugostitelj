...
export function HostviaHeroMockup() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-xl lg:max-w-none">
      <div className="hostvia-pulse-glow absolute -inset-8 rounded-full bg-violet-500/20 blur-3xl" />
      <div className="hostvia-float relative space-y-4">
        {/* Dashboard card */}
        <div className="hostvia-glow-card relative overflow-hidden p-5 shadow-2xl">
          ...
          <div className="mb-2 grid grid-cols-7 gap-1">
            {days.map((d, i) => (
              <div
                key={i}
                className="text-center text-[9px] font-medium text-zinc-500"
              >
                {d}
              </div>
            ))}
          ...
          <div className="grid grid-cols-7 gap-1">
            {calendarCells.map((cell, i) => (
              <div
                key={i}
                className={`relative flex h-7 items-center justify-center rounded-md text-[10px] font-medium ${
                  cell.checkIn
        ...
        {/* Booking website preview — offset */}
        <div className="absolute -bottom-6 -right-4 w-[55%] sm:-right-8">
          <div className="hostvia-glow-card overflow-hidden shadow-2xl">
            <div className="h-16 bg-gradient-to-br from-violet-600/40 via-indigo-600/30 to-cyan-600/20" />
            <div className="space-y-2 p-3">
              ...
              <div className="flex gap-1">
                {[1, 2, 3].map((n) => (
                  <div
                    key={n}
                    className="h-8 flex-1 rounded bg-white/10"
                  />
                ))}
              </div>
